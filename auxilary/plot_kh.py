...
""" 
This code section compare the DR between 3 methods and return the minimal one in a plot.
"""
a = [9 / 8, 1 - 3 * model1.trainable_weights[0], 1 - 3 * calculate_DRP2()]
if __name__=='__main__':
    ...
    for i, kx in enumerate(Constants.K1_TRAIN):
        for j, ky in enumerate(Constants.K2_TRAIN):
            ...
            if i == j:
                ...
                dl.append(Z[1])
                dp.append(Z[2])
                dx.append(kx * Constants.DX)

    fig, axs = plt.subplots(nrows=1, ncols=2)
    axs[0].plot(dx, d4, color='red', label='Yee4')
    axs[0].plot(dx, dl, color='green', label='DL2')
    axs[0].plot(dx, dp, color='blue', label='DRP')
    axs[0].set(title="Dispersion error", xlabel=r'${ \frac{hk_x}{\pi}  }$', ylabel='Error')
    # axs[0].xlabel(r'${ \frac{hk_x}{\pi}  }$', fontsize=12)
    # axs[0].ylabel(r'${ \mathrm{Error}}$', rotation=0, fontsize=12, labelpad=14)

    # axs[0].axis('square')

    axs[0].legend()
    # axs[0].savefig('/Users/idanversano/documents/papers/drp/figures/phasekk_31.eps', format='eps',
    #            bbox_inches='tight')

    cMap = ListedColormap(['red', 'green', 'blue'])
    im = pcolormesh(Constants.K1_TRAIN * Constants.DX, Constants.K1_TRAIN * Constants.DX, com, cmap=cMap)
    ...
    axs[1].legend(handles=[pop_a, pop_b, pop_c])
    axs[1].set(title="Dispersion minimizers", xlabel=r'${ \frac{hk_x}{\pi}  }$', ylabel=r'${ \frac{hk_y}{\pi}  }$')
    axs[1].set(adjustable='box', aspect='equal')
    axs[1].axis('square')
    plt.tight_layout()
    # axs[1].suptitle("Dispersion relation minimizers", size=12, y=0.98)
    ...
    # Adding grid to plot
    # plt.savefig('/Users/idanversano/documents/papers/drp/figures/phase3_31.eps', format='eps',
    #             bbox_inches='tight')

    fig.savefig('/Users/idanversano/documents/papers/drp/figures/phase2_31.eps', format='eps',
                bbox_inches='tight')
    plt.show()
