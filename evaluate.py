...
for i in range(len(test_data['e'])):
    ...
    l_model2.append(loss_yee('model', 0, model2.trainable_weights[0], data))
    l_yee.append(loss_yee('Yee', 0, 0, data))
    l_drp.append(loss_yee('DRP', 0., -0.20346644, data))
...
plt.plot(l_model2, 'b', label='modeli_i')
plt.plot(l_model, 'g', label='model_c')
